...
local function ReadableNumber(num, places)
    ...
    elseif num >= 10000000000000 then
        ret = placeValue:format(num / 1000000000000) .. " t" -- tetra
    elseif num >= 10000000000 then
        ret = placeValue:format(num / 1000000000) .. " g" -- giga
    elseif num >= 10000000 then
        ret = placeValue:format(num / 1000000) .. " m" -- mega
    elseif num >= 10000 then
        ret = placeValue:format(num / 1000) .. "k" -- kilo
    else
        ret = num -- hundreds
    end
    return ret
...
local function ceil(value, num)
 ...
 function FTC.Frames:UpdateAttribute( unitTag , powerType ,  powerValue , powerMax , powerEffectiveMax )
	...
	-- Get the percentage
	-- 0.01 should become -> 0.1 not 0.0 --> ceil instead of floor
	local pct = ceil( ( powerValue / powerMax ) * 100, 1 )
	local powerValueFormatted = ReadableNumber(powerValue, 1)
	local powerMaxFormatted = ReadableNumber(powerMax, 1)
	local powerEffectiveMaxFormatted = ReadableNumber(powerEffectiveMax, 1)
	...
	if ( FTC.init.Frames ) then
		...
		if(context ~= "Target") then
			...
			curLabel:SetText( powerValueFormatted .. "/" .. powerMaxFormatted )
			pctLabel:SetText(pct .. "%")
		else
			curLabel:SetText( pct .. "% (" .. powerValueFormatted .. "/" .. powerMaxFormatted .. ")")
		end
	...
	if ( FTC.vars.FrameText ) then
		...
		default:SetText( powerValueFormatted .. " / " .. powerMaxFormatted .. " (" .. pct .. "%)")
	end
