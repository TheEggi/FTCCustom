...
function FTC.Damage:UpdateMeter( newDamage , context )
	...
	elseif ( context == "Out" ) then
		...
		meter.group				= meter.group + damage
		...
		if ( IsUnitGrouped('player') ) then
			FTC.Damage.Group[source] = ( FTC.Damage.Group[source] ~= nil ) and FTC.Damage.Group[source] + damage or damage
		end
...
function FTC.Damage:Display()
	...
	-- Generate a title
	if ( meter.group == 0 and meter.healing == 0 ) then
...
function FTC.Damage:UpdateMini() 
	...
	if ( IsUnitGrouped('player') ) then
		...
		-- Sort group damage
		local damages	= {}
		for k,v in pairs( FTC.Damage.Group ) do
			player		= { ["name"] = k , ["dam"] = v }
			table.insert( damages , player ) 
		end
		table.sort( damages , function(x,y) return x.dam > y.dam end )
		
		-- Construct the label
		for i = 1 , #damages do
	
			-- Format the group member name
			local name	= SanitizeLocalization( damages[i].name )
			local a,b 	= string.find( name , "%s" )
			if ( a ~= nil ) then name = string.sub( name , 1 , a - 1 ) end
	
			-- Get group member dps
			local gdps 	= string.format( "%.2f" , damages[i].dam / fight_time )
			
			-- Generate a label
			label		= label .. name .. " - " .. gdps .. "\r\n"
			gCount		= gCount + 1
		end
		
		-- Update label and set height
		local height = ( gCount >= 1 ) and 35 + ( gCount * 20 ) or 30
		mini.group:SetHeight( height - 30 )
		mini.group:SetText( label )
		mini.group:SetHidden(false)
		mini:SetHeight( height )
		mini.backdrop:SetHeight( height )
	
	-- Otherwise hide group
	else 
		mini:SetHeight( 30 )
		mini.group:SetHidden(true) 
	end
...
function FTC.Damage:Post()
	...
	-- Generate output
	local label = name .. " (" .. string.format( "%.1f" , fight_time ) .. "s) - " .. CommaValue(meter.group) .. " Damage"
	local dps = string.format( "%.1f" , meter.damage/fight_time)
	label = label .. " || " .. "(" .. dps .. " Total DPS)"
	...
	if ( IsUnitGrouped('player') ) then
		...
		for i = 1 , #damages do
			...
			-- Append the label
			-- TODO: check if meter.damage or damages[i].dam should be used? damages[i].dam looks right
			label = label .. " || "	.. name .. " - " .. CommaValue(damages[i].dam) .. " (" .. gdps .. " DPS)"
		end	
	
	end
	-- Compute healing
	local hps = string.format( "%.1f", meter.healing/fight_time)
	label = label .. " || " .. "Healing " .. meter.healing .. "(" .. hps .. " HPS)"
